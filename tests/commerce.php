...
foreach(['app/Services/OrderService.php','app/Services/Cart.php','app/Services/Shipping.php','app/Payments/IyzicoGateway.php','app/Payments/GatewayFactory.php','app/Controllers/CheckoutController.php','app/Controllers/PaymentController.php','app/Services/PdfPriceList.php','db/migrations/20260906_005_commerce.sql','db/migrations/20260906_005b_payment_identity.sql'] as $f){if(!is_file($root.'/'.$f))$fail[]='Eksik: '.$f;}
$order=(string)@file_get_contents($root.'/app/Services/OrderService.php');foreach(['FOR UPDATE','stock=stock-?','stock=stock+?','stock_released','coupons','identity_number'] as $needle){if(!str_contains($order,$needle))$fail[]='OrderService kuralı eksik: '.$needle;}
$shipping=(string)@file_get_contents($root.'/app/Services/Shipping.php');if(!str_contains($shipping,'shipping_rules'))$fail[]='Shipping servisinde kargo kuralı sorgusu eksik.';
...
$payment=(string)@file_get_contents($root.'/app/Controllers/PaymentController.php');foreach(['payment_status','provider_token',"payment_status<>'paid'"] as $needle){if(!str_contains($payment,$needle))$fail[]='Ödeme hata/tekrar koruması eksik: '.$needle;}foreach(['grand_total','paid_price'] as $needle){if(!str_contains($payment,$needle))$fail[]='Ödeme tutar doğrulaması eksik: '.$needle;}
$ignore=(string)@file_get_contents($root.'/.gitignore');if(!str_contains($ignore,'/integrations/iyzipay/'))$fail[]='Resmi SDK kopyası gitignore dışında.';
