...
final class Installer
{
    ...
    public static function run(array $config, string $email, string $password): void
    {
        ...
        $pdo = new PDO($dsn, (string)$db['user'], (string)$db['pass'], [PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_EMULATE_PREPARES => false]);
        $pdo->exec((string)file_get_contents(ARCATES_ROOT . '/schema.sql'));
        $stmt = $pdo->prepare('INSERT INTO users (email, password_hash, role, is_active, created_at) VALUES (?, ?, ?, 1, NOW())');
        $stmt->execute([mb_strtolower(trim($email)), password_hash($password, PASSWORD_DEFAULT), 'admin']);
