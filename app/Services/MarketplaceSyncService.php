...
final class MarketplaceSyncService
{
    ...
    {
        $gateway=MarketplaceFactory::make($provider);$claim=bin2hex(random_bytes(16));$items=$this->claimChanged($provider,$claim,$gateway->maxBatchSize());if($items===[])return ['provider'=>$provider,'submitted'=>0,'batch_id'=>null];
