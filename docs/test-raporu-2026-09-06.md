...
| BULGU-03 (hız limiti) | `67ffb17`, `1d11cf8`, saat dilimi düzeltmesi | ⚠️→✅ **İki aşamada kapandı.** Önce `rate_limit_buckets` tablosu eklenip `RateLimiter` kalıcı depoya taşındı. Ancak pencere karşılaştırması PHP `time()`/`strtotime()` ile MySQL `NOW()` damgasını kıyaslıyordu. MySQL ile PHP farklı saat diliminde çalıştığında (yaygın kurulum: MySQL UTC, `app.timezone` Europe/Istanbul) sabit 3 saatlik kayma oluşuyor ve pencere **her çağrıda dolmuş görünüyor**: `hits` 1'e sıfırlanıyor, limit hiç devreye girmiyor. Yani limit kalıcı depoya taşınmasına rağmen fiilen hâlâ devre dışıydı. Karşılaştırma tamamen SQL tarafına alındı (`window_started_at <= NOW() - INTERVAL n SECOND`). Bu hatayı CI'daki gerçek MySQL runtime testi yakaladı — statik metin kontrolleri göremezdi. |
